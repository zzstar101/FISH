...
import { tryLockUserWrites } from './lock'
 ...
 * 其它位置一律引用它，不复制这段 SQL。
 *
 * 用 `clock_timestamp()` 而不是 `now()`（事务开始时间）：治理事务可能在用户行锁上
 * 等过到期边界；取得锁之后的判定必须使用当前 DB 时钟。也不能用 API 主机时钟。
 */
export const ACTIVE_RESTRICTION_WHERE = sql`${userRestrictions.status} = 'ACTIVE'
  AND (${userRestrictions.expiresAt} IS NULL OR ${userRestrictions.expiresAt} > clock_timestamp())`
...
const EXPIRED_RESTRICTION_WHERE = sql`${userRestrictions.status} = 'ACTIVE'
  ...
  AND ${userRestrictions.expiresAt} <= clock_timestamp()`

export interface GovernanceStore {
  /** 持有与治理动作相同的用户级事务锁，覆盖限制检查到写请求完成的整个窗口。 */
  withWriteGuard(
    userId: string,
    scope: RestrictionScope,
    write: () => Promise<void>,
    signal?: AbortSignal,
  ): Promise<'allowed' | 'restricted' | 'busy'>
   ...
   * `clock_timestamp()` 也非 immutable）。于是一条已过期的行会继续占用唯一槽位——service 视角它
...
export function createSqlGovernanceStore(_db: Db): GovernanceStore {
  return {
    async withWriteGuard(userId, scope, write, signal) {
      // Try and release the connection rather than stacking blocking lock waiters in the pool.
      // An abandoned request or prolonged write must not leave unlimited queued DB traffic.
      const deadline = performance.now() + 10_000
      let delay = 25
      for (;;) {
        if (signal?.aborted || performance.now() >= deadline) return 'busy'

        const result = await _db.transaction(async (tx) => {
          const [lock] = rowsOf(await tx.execute(tryLockUserWrites(userId)))
          if (!lock?.locked) return 'busy' as const
          const typeCondition =
            scope === 'publish' ? sql`type IN ('PUBLISH_RESTRICT', 'BAN')` : sql`type = 'BAN'`
          const active = await tx.execute(sql`
            SELECT id FROM user_restrictions
            WHERE user_id = ${userId} AND ${ACTIVE_RESTRICTION_WHERE} AND ${typeCondition}
            LIMIT 1
          `)
          if (rowsOf(active).length > 0) return 'restricted' as const
          await write()
          return 'allowed' as const
        })
        if (result === 'busy') {
          await Bun.sleep(Math.min(delay, Math.max(0, deadline - performance.now())))
    ...
    async insertRestriction(tx, input) {
      const result = await tx.execute(sql`
        INSERT INTO user_restrictions
          (user_id, type, status, reason, actor_user_id, source_report_id, expires_at, created_at, updated_at)
        VALUES
          (${input.userId}, ${input.type}::user_restriction_type, 'ACTIVE',
