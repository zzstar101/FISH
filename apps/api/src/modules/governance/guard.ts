...
export function createRestrictionGuard(options: {
  store: Pick<GovernanceStore, 'withWriteGuard'>
}): RestrictionGuard {
  const guard =
    ...
    async (c, next) => {
      ...
      const result = await options.store.withWriteGuard(userId, scope, next, c.req.raw.signal)
      if (result === 'restricted') return c.json(errorBody('USER_RESTRICTED', MESSAGES[scope]), 403)
      if (result === 'busy') {
        c.header('Retry-After', '1')
        return c.json(errorBody('USER_GUARD_BUSY', '操作繁忙，请稍后重试'), 503)
      }
    }
