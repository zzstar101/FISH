...
export function createGovernanceService(options: {
  ...
  /** `sourceReportId` 是可选外链；传了就必须存在，否则等于把处罚挂到不存在的举报上。 */
  async function requireSourceReport(
    ...
    const result = await tx.execute(sql`
      SELECT id FROM reports WHERE id = ${sourceReportId} LIMIT 1
    `)
  ...
  /** 锁住用户行并确认存在。`FOR UPDATE` 把同一用户上的治理动作串行化。 */
  ...
  return {
    async delistListing(actorUserId, listingId, input) {
      return db.transaction(async (tx) => {
        ...
        if (current.moderationStatus === 'BLOCKED') {
        ...
        const updated = await tx.execute(sql`
          ...
          WHERE id = ${listingId} AND moderation_status <> 'BLOCKED'
...
async function priorListingStatus(
  tx: GovernanceDbTransaction,
  listingId: string,
): Promise<'ACTIVE' | 'RESERVED' | 'SOLD' | 'OFFLINE'> {
  const result = await tx.execute(sql`
  ...
  if (prior === 'ACTIVE' || prior === 'RESERVED' || prior === 'SOLD' || prior === 'OFFLINE') {
    return prior
  }
  return 'ACTIVE'
}
