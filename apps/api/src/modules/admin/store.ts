...
export type ModerationHistoryRow = Omit<ModerationQueueRow, 'listing'> & {
  listing: ModerationListingRow | null
}
...
export interface AdminStore {
   ...
   */
  listModerationRecords(criteria: ListModerationRecordsCriteria): Promise<ModerationHistoryRow[]>
  ...
  /** 目标最近操作；USER 也包含通过 user_restrictions 关联的处罚／解除审计。 */
  recentAuditLogs(
  ...
  return {
    ...
    async listModerationRecords(criteria) {
      ...
      return rowsOf(result).map((row) => ({
        ...
        createdAtCursor: String(row.created_at_cursor),
        record: moderationRecordFromRow(row),
        listing: row.listing_id == null ? null : moderationListingFromRow(row),
        seller: { id: String(row.seller_id), nickname: String(row.seller_nickname) },
      }))
    },
    ...
    async getModerationDetail(recordId) {
      const initialIdResult = await db.execute(sql`
        ...
        LEFT JOIN id_rekeys k ON k.resource_table = 'listing_moderation_records'
          AND k.old_id::text = a.after->>'manualRecordId'
        WHERE a.action = 'MODERATION_DECISION'
          AND a.target_type = 'MODERATION_RECORD'
          AND (a.after->>'manualRecordId' = ${recordId} OR k.new_id = ${recordId}::uuid)
    ...
    async recentAuditLogs(targetType, targetId, limit) {
      return db
        ...
        .where(
          targetType === 'USER'
            ? sql`(${adminAuditLogs.targetType} = 'USER' AND ${adminAuditLogs.targetId} = ${targetId})
              OR (${adminAuditLogs.targetType} = 'USER_RESTRICTION' AND EXISTS (
                SELECT 1 FROM ${userRestrictions}
                WHERE ${userRestrictions.id} = ${adminAuditLogs.targetId}
                  AND ${userRestrictions.userId} = ${targetId}
              ))`
