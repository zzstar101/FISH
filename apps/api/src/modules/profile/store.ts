...
/** 我发布的商品行（含封面 objectKey；URL 由 service 经 MediaStorage 拼）。 */
...
export function createSqlProfileStore(db: Db): ProfileStore {
  return {
      ...
      const result = await db.execute(sql`
        SELECT l.id, l.title, l.price_cents, l.category::text AS category,
               ...
               (SELECT li.object_key FROM listing_images li
                 WHERE li.listing_id = l.id ORDER BY li.sort_order ASC LIMIT 1)
                 AS cover_object_key
        FROM listings l
    ...
    async ownWishes(userId, limit) {
      // category 可空（#2 为 #8 预留「不限分类」）：契约的 WishDto.category 非空，
      // 这里过滤 NULL 行——与 wishes 模块需求池聚合的同一口径（需求池过滤 NULL 分类）。
      const result = await db.execute(sql`
        ...
        FROM wishes w
        WHERE w.user_id = ${userId} AND w.category IS NOT NULL
