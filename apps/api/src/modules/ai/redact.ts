...
export type Redactor = {
  ...
  /**
   * 回填。整条降级而非逐标记混合（设计 §5.7）：任一标记没原样回来，或出现不是本实例发出的
   * 标记（模型自造），该条全部标记位都换成类型化提示语——绝不按顺序猜，猜错等于把 A 的电话
   * 接到 B 的位置。
   */
  restore(text: string): { text: string; lost: boolean }
...
export function createRedactor(): Redactor {
  ...
  return {
    ...
    restore(text) {
      const present = new Set(text.match(MARKER_PATTERN) ?? [])
      const foreign = [...present].some((marker) => !mapping.has(marker))
      const missing = [...mapping.keys()].some((marker) => !present.has(marker))
      if (foreign || missing) {
        return {
          text: text.replace(MARKER_PATTERN, (_match, kind: string) => promptFor(kind)),
          lost: true,
        }
      }
      return {
