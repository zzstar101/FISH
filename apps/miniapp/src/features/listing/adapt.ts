...
/**
 * 详情页的卖家：契约 `ListingDetail.seller` 有 `id / nickname / avatarUrl / campus`，
 * 但**刻意没有** `authStatus`（见 `packages/contracts/src/listings/schema.ts` 的注释：
 * 当前 `authStatus` 来自 Mock Provider，不可作为信任依据，因此不进商品详情）。
 *
 * 所以真实数据下：`authStatus` 取 `UNVERIFIED`（不显示认证勾，而不是假装已认证），
 * `soldCount` / `goodRate` 契约没有 —— 置 `null` 由页面不渲染。
 * 这两个字段在 `MockUser` 上是必填的，这里改成可空（见 types.ts）。
 */
export function toMockSeller(detail: ListingDetail): MockUser {
  return {
    id: detail.seller.id,
    ...
    // 契约不带认证状态：不显示徽章，避免把 Mock 时代的结论当可信依据
