 ...
 * ## 「评价对象」与「时间」必须与订单页对得上
 *
 * 稿子里这两列是随手写的占位，照抄会与订单页**自相矛盾**，所以按 fixture 的真实值改过：
 *
 * | 行 | 引用的成交 | 对方 | 完成时间（订单页「已于 …」） | 评价时间 |
 * | --- | --- | --- | --- | --- |
 * | C05 | t-104 米家台灯 | 周予安 | 2026-08-19 | 8 月 21 日 |
 * | C06 | t-106 尤尼克斯球拍 | 许澈 | 2026-08-12 | 8 月 14 日 |
 * | C07 | 无（占位） | 橙子 | — | 5 月 20 日 |
 * | C08 | 无（占位） | 苏打水 | — | 4 月 28 日 |
 *
 * - **对方取商品归属人**（`@对方` 说的就是这笔交易的另一方）：C07 那套《灌篮高手》在
 *   fixture 里属于 `u-chengzi`（橙子，`mock/catalog.ts`），稿子写的「苏亦然」是另一位
 *   用户（`u-suyiran`，卖的是苹果鼠标 / 佳能单反）—— 照抄就会在橙子的商品上 `@苏亦然`。
 * - **评价时间必须在成交之后**：C05/C06 稿里写的是 5 月 12 日 / 5 月 6 日，而这两笔交易
 *   的完成时间是 8 月下旬 —— 会显示成「面交之前两个月就评价了」。已按完成时间顺延。
 *
 * ## 剩下两条（C07/C08）确实是演示占位
 *
 * 它们**没有**对应的成交：演示库里可评的已完成交易只有 4 笔（`mock/account.ts` 的
 * t-104 / t-106 卖出侧，t-107 索尼 / t-108 高数 买入侧），而这两张卡的商品是稿子里另外
 * 两件。所以这两条给不出真实出处 —— 这条口径要一并交给后端确认，别让「演示里能评、
 * 真实账户里一笔可评的都没有」变成上线后的落差（已记入 Issue #195）。
 */
