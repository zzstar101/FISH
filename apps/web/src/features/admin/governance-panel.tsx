...
export function GovernancePanel({
  ...
  const submit = (spec: GovernanceActionSpec) => {
    ...
    governance.mutate(
      { action: spec.action, targetId, input: { reason: reason.trim(), sourceReportId } },
  ...
  return (
    <Card className="space-y-3 p-4">
      ...
      {sourceReportId ? (
        <p className="rounded-lg border border-brand/30 bg-brand/5 px-3 py-2 text-sm text-ink-2">
          本次操作将关联{' '}
          <Link
            className="font-medium text-brand underline"
            params={{ reportId: sourceReportId }}
            to="/admin/reports/$reportId"
          >
            来源举报
          </Link>
          ；如需独立巡查处理，请从列表重新进入目标详情。
        </p>
      ) : null}
