...
| `AI_POLISH_BASE_URL` | live 时 | 形如 `https://api.deepseek.com` |
...
实现方式：`EMPTY` 行照样写（供质量指标用），但两条检查的 COUNT/MAX 加 `outcome <> 'EMPTY'` 过滤。
...
| c | **数字 / 单位 ⊆ 校验**：原文与候选各自归一化后取"数字 + 紧邻单位"有序集合，候选 ⊄ 原文 → 丢 | 这是"AI 不得新增事实"唯一可验证的形式。归一化＝NFKC + 剥空白 + 中文数字→阿拉伯；**不做单位换算表**（第二个会漂移的真相源） |
