...
- `apps/api/src/modules/wishes/router.ts`：相对路由；已由 app.ts 以 `app.use('/wishes/*', auth.requireAuth)` + `app.route('/wishes', ...)` 挂载在**根级**（见 `apps/api/src/app.ts`、`packages/contracts/src/wishes/routes.ts`）。
- 集成测试：`store.test.ts` 与 `app.wishes.test.ts` 均以 per-pid scratch 库跑真实 migration（Windows 下必须用 `fileURLToPath`，勿用 `URL.pathname`）。

> **路径口径收敛（#43 → #53 → #55）**：本文件此前把**浏览器前缀**写进了 API 路径（`/api/wishes`），
> 与仓约定「API 路由保持根级，Web 写相对路径 `/api` + 常量、由 Vite 代理去掉前缀」
> （`docs/architecture.md` §5.1，`listings` / `matching` 同款）不一致。现已收敛为根级 `/wishes`，
> `apps/api/src/app.ts` 只挂这一个路径，`/api/wishes` 的过渡别名已删除（PR #55）。
...
> **下表是 API 侧路径（根级）**，与 `listings` / `matching` 的 `routes.ts` 同口径。
> Web 侧写相对路径 `/api` + 契约常量，由 Vite 代理去掉 `/api` 前缀后落到这里
> （`docs/architecture.md` §5.1）。真值来源是 `packages/contracts/src/wishes/routes.ts`，
> 浏览器侧完整路径 = `/api` + 常量（例如 `/api/wishes`）。
...
**1.2 `packages/contracts/src/wishes/routes.ts`**：路径常量（API 侧根级：`/wishes`、`/wishes/:id`、`/wishes/:id/close`、`/wishes/:id/fulfill`、`/wishes/pool`）。
...
zzstar101 在 `app.ts` 中挂载 `app.route('/wishes', wishesRouter)`（**根级**，见 §3 的路径口径说明）。合并顺序：contracts PR #A → API PR #B → 接线改动（可由 zzstar101 在其集成 PR 中带上）。
